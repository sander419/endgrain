...
Тесты — `npm test` (Vitest, 167 тестов).
...
Прод отдаётся nginx с HSTS, `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`,
`Referrer-Policy: strict-origin-when-cross-origin` и `Permissions-Policy` с выключенными
камерой, микрофоном и геолокацией. Прод-зависимостей две — `react` и `react-dom`,
`npm audit` чист.
